            ...
            foreach (var userId in _data.Keys)
            {
                ...
                if (userData.Choice == _currentQuestion!.RightAnswerIndex)
                {
                    ...
                    if (player != null)
                    {
                        ...
                        await _mathQuestionHub.Clients.User(userId).SendAsync("bonnesRéponses");
                    }
                }
                else
                {
                    var bonneRéponse = _currentQuestion!.Answers[_currentQuestion!.RightAnswerIndex];
                    await _mathQuestionHub.Clients.User(userId).SendAsync("mauvaisesRéponses", bonneRéponse);
                }
